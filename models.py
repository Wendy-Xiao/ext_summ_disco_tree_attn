...
class Bert(nn.Module):
	...
	def forward(self, x, segs, mask,return_attention=False):
		if return_attention:
			last_hidden_state, _, attentions = self.model(x, token_type_ids=segs, attention_mask =mask, output_attentions=True)
			return last_hidden_state,attentions
		else:
			last_hidden_state, _ = self.model(x, token_type_ids=segs, attention_mask =mask)
...
class DiscoExtSumm(nn.Module):
	def __init__(self, args, load_pretrained_bert, bert_config=None):
		super(DiscoExtSumm, self).__init__()
		...
		self.Transformer = Encoder(n_layers, n_head, d_k, d_v,768, d_inner,attention_type=self.attention_type)
	...
	def forward(self, batch,device):
		...
		if self.attention_type=='tree':
			out = self.Transformer(inputs, batch.unit_mask, tree_attn=batch.attn_map)
		else:
			out = self.Transformer(inputs,batch.unit_mask)
	
		out = self.Dropoutlayer(out)
		out = self.Decoderlayer(out) # batch * length * 1

		return out,unit_repre
